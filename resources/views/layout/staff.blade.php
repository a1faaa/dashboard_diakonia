...
<body>
        ...
        <ul class="sidebar-nav" data-coreui="navigation" data-simplebar="">
            ...
            <li class="nav-item mt-2">
                <a class="nav-link <?= isset($sidebar) && $sidebar == 'kegiatan' ? 'active' : '' ?>"
                    href="{{ route('master.kegiatan.index') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="nav-icon" viewBox="0 0 512 512">
                        <path fill="var(--ci-primary-color, currentColor)"
            ...
            <li class="nav-item mt-auto">
                <a class="nav-link <?= isset($sidebar) && $sidebar == 'anggaran' ? 'active' : '' ?>"
                    href="{{ route('master.anggaran.index') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="nav-icon" viewBox="0 0 512 512">
                        <path fill="var(--ci-primary-color, currentColor)"
                            d="M334.627,16H48V496H472V153.373ZM440,166.627V168H320V48h1.373ZM80,464V48H288V200H440V464Z"
                            class="ci-primary" />
            ...
            <li class="nav-item mt-auto">
                <a class="nav-link <?= isset($sidebar) && $sidebar == 'proker' ? 'active' : '' ?>"
                    href="{{ route('master.proker.index') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="nav-icon" viewBox="0 0 512 512">
                        <path fill="var(--ci-primary-color, currentColor)"
                            d="M334.627,16H48V496H472V153.373ZM440,166.627V168H320V48h1.373ZM80,464V48H288V200H440V464Z"
                            class="ci-primary" />
                        <rect width="224" height="32" x="136" y="296"
                            fill="var(--ci-primary-color, currentColor)" class="ci-primary" />
                        <rect width="224" height="32" x="136" y="376"
                            fill="var(--ci-primary-color, currentColor)" class="ci-primary" />
                    </svg> Proker
                </a>
            </li>
            <li class="nav-item mt-auto">
                <a class="nav-link <?= isset($sidebar) && $sidebar == 'user' ? 'active' : '' ?>"
                ...
                <ul class="header-nav">
                    <li class="nav-item dropdown"><a class="nav-link py-0 pe-0" data-coreui-toggle="dropdown"
                        ...
                        <div class="dropdown-menu dropdown-menu-end pt-0">
                            ...
                            </div><a class="dropdown-item" href="{{ route('staff.profile') }}">
    ...
    <script>
        ...
        // Function to remove dots for storage
        ...
        function sanitizeInput(value) {
            return value.replace(/[^0-9]/g, ""); // Allow only digits (0-9)
        }

        // Initialize number formatting on input fields with the class 'format-number'
        function initializeNumberFormatting() {
            $('.format-number').on('input', function() {
                ...
                // Remove dots for consistent formatting
                value = unformatNumber(value);

                // Format and set the value back to the input field
                $(this).val(formatNumber(value));
            });

            // On form submission, remove formatting for storage
            $('form').on('submit', function() {
                $(this).find('.format-number').each(function() {
                    let inputField = $(this);
                    let plainValue = unformatNumber(inputField.val());

                    // Set the plain number back before submitting
                    inputField.val(plainValue);
                });
            });
        }
